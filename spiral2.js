...
Spiral.prototype.step = function() {
    ...
    this.sequence.push(sin(next * 0.00001) * 30000);
    this.index = next;
    this.counter++;
};
...
Spiral.prototype.showSpiral = function() {
    ...
    for (let i = 0; i < 125; i++) {
        let d = this.drawnIndex;
        let x = cos(this.sequence[d]) * this.sequence[d] * 0.05;
        let y = sin(this.sequence[d]) * this.sequence[d] * 0.05;
        let radius = map(this.sequence[d], 0, 50000, 2, 10);
        ellipse(x, y, radius);
        this.drawnIndex++;
    }
